...
def save_round(round, matches):
    ...
    for match_id, players in matches.items():
        ...
        if match.sgf is None:
            if len(players) > 0:
                player_1 = get_object_or_404(TournamentPlayer, pk=players[0], event=round.tournament)
                match.player_1 = player_1
                match.player_2 = None
                if len(players) == 2:
                    player_2 = get_object_or_404(TournamentPlayer, pk=players[1], event=round.tournament)
